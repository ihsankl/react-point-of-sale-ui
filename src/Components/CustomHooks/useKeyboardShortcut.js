...
const useKeyboardShortcut = (
  ...
  if (!Array.isArray(shortcutKeys)) {
    throw new Error(
        `The first parameter to 'useKeyboardShortcut'
        must be an ordered array of 'KeyboardEvent.key' strings.`,
    );
  }
  ...
  if (!shortcutKeys.length) {
    throw new Error(
        `The first parameter to 'useKeyboardShortcut'
        must contain atleast one 'KeyboardEvent.key' string.`,
    );
  }

  if (!callback || typeof callback !== 'function') {
    throw new Error(
        `The second parameter to 'useKeyboardShortcut'
        must be a function that will be envoked when the keys are pressed.`,
    );
  }

  // Normalizes the shortcut keys a deduplicated array of lowercased keys.
  const shortcutArray = useMemo(
      () => uniqFast(shortcutKeys).map((key) => String(key).toLowerCase()),
      // While using JSON.stringify() is bad
      //   for most larger objects, this shortcut
